...
def inject_players_csv(template_path: str, rows: list[dict], out_stream, shell_mode: bool = True):
    ...
    # clear the non-Players sheets so the download is a shell
    if shell_mode:
        for name in SHELL_CLEAR_SHEETS:
            if name in wb.sheetnames:
                _clear_data_below_header(wb[name])
    ...
    for row in rows:
        for h in PLAYER_HEADERS:
            ws.cell(row=r, column=header_map[h], value=row.get(h, ""))
        r += 1

    # GameTally: ensure Notes width = 30
