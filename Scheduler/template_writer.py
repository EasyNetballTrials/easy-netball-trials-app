...
def _normalize_players_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    If 'Number' is missing in the upload CSV, insert a blank 'Number' at the 4th position.
    All non-present columns are created as blank strings.
    """
    ...
    # If Number was missing, we already created it above; ensure precise order
    df = df.loc[:, list(EXPECTED_ORDER)]

    # Coerce to strings where appropriate; keep numbers/seeds as text so Excel shows what VBA expects
    for col in ["First Name", "Last Name", "Active",
                "PreferredPos1", "PreferredPos2", "PreferredPos3"]:
        df[col] = df[col].fillna("").astype(str)

    # Let Seed/Number pass through (as text) — VBA will handle numbering later if needed
...
def _write_dataframe_to_sheet(ws, df: pd.DataFrame, start_row: int = 1, start_col: int = 1):
    """
    Write a small DataFrame to an openpyxl worksheet starting at (start_row, start_col),
...
def write_into_template(
    ...
    - Normalizes Players columns (inserts blank Number if missing)
    ...
    # Optional cleanups for a tidy download
    ...
    # Basic autofit (approximate: set width by longest header/value length)
    for j, col in enumerate(df_norm.columns, start=1):
        # choose a reasonable width based on header and some sample data
        header_len = len(col)
    ...
    ws_control["B4"].value = "YES" if numbering else "NO"   # Numbering switch (if your VBA reads this)
    ws_control["B6"].value = "YES" if show_prefs else "NO"  # Show preferred positions on CourtBoards
